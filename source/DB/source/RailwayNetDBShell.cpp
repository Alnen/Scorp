...
/*
std::vector<int> RailwayNetDBShell::getRoutes(int stationFrom, int stationTo, const ScorpCore::Time& timeFrom, const ScorpCore::Time& timeTo)
{
    std::vector<RailwayNetDBObject::RoutePart> route_part_list;
    std::vector<RailwayNetDBObject::Route> route_list;
    std::vector<RailwayNetDBObject::Transition> transition_list;
    ...
    try
    {
        SQLite::Statement query(*m_database, "SELECT Transition.* FROM Transition, RoutePart WHERE (Transition.Id = RoutePart.Transition) AND ((Transition.StationFrom = :stationFrom) AND (RoutePart.TimeOffsetFrom >= :timeFrom))");
        query.bind(":stationFrom", stationFrom);
        query.bind(":timeFrom", timeFrom);
        while (query.executeStep())
        {
            tr_list.push_back(query.getColumn(0).getInt());
        }
    }
    catch (SQLite::Exception)
    {
        std::cout << "[getTransitionFrom] Some Error" << std::endl;
...
/*
SELECT * FROM Transition WHERE (Transition.Id = RoutePart.Transition) AND ((Transition.StationFrom = :stationFrom OR Transition.StationTo = :stationTo) AND RoutePart.Id =

SELECT Route.Id
FROM Route
WHERE EXISTS (SELECT *
              FROM Transition, RoutePart
              WHERE Route.Id = RoutePart.Route AND Transition.TimeOffsetFrom <= :timeFrom AND Transition.time);
 AND EXISTS (SELECT *
      FROM Transition, RoutePart
      WHERE Route.Id = RoutePart.Route);
)
*/
/*
std::vector<RailwayNetDBObject::Route> RailwayNetDBShell::getRoutes(int station_1, int station_2)
{
    std::vector<RailwayNetDBObject::Route> route_list_1;
    std::vector<RailwayNetDBObject::Route> route_list_2;
    int curr_id = -1;
    try
    {
        SQLite::Statement query(*m_database, std::string("SELECT Route.Id FROM Transition,")
            + " RoutePart, Route WHERE Transition.StationFrom = :stationFrom AND"
            + " Transition.Id = RoutePart.Transition AND Route.Id = RoutePart.Route");
        while (query.executeStep())
        {
            curr_id = query.getColumn(0).getInt();
            route_list.push_back(RailwayNetDBObject::Route(curr_id,
...
//"SELECT Route.Id FROM Transition, RoutePart, Route WHERE Transition.StationFrom = :stationFrom AND Transition.Id = RoutePart.Transition AND Route.Id = RoutePart.Route"
//"SELECT Route.Id FROM Transition, RoutePart, Route WHERE Transition.StationFrom = :stationTo AND Transition.Id = RoutePart.Transition AND Route.Id = RoutePart.Route"
